...
This initial analysis aims to see how the percentage of votes for the Democratic Presidential candidate, Harris, has changed over time for various high-quality pollsters. 
...
```
@fig-analysis shows that the percentage of votes for Harris has been steady or has slightly increased over time across various pollsters. 
...
ggplot(data, aes(x = created_at, y = pct)) +
  ...
  theme(legend.position = "bottom")
```
@fig-states shows that support for Harris within each state remained fairly constant over time with minimal fluctuations. This also seems to be the case for the percentage of votes for Harris with the national polls.
